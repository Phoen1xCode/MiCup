...
_REGISTRY: dict[str, dict] = {
    ...
    "football": dict(
        type="obj",
        topic="/perception/football",
        hsv_key="football_dark",
        label="football",
        width=0.2,
    ),
    "coke": dict(
        type="obj", topic="/perception/coke", hsv_key="coke", label="coke", width=0.12
    ),
    "red_pole": dict(
        type="obj",
        topic="/perception/red_pole",
        hsv_key="red_pole",
        label="red_pole",
        width=0.1,
    ),
    "block_obstacle": dict(
        type="obj",
        topic="/perception/block_obstacle",
        hsv_key="gray_block",
        label="block",
        width=0.2,
    ),
...
def run(detector_name: str, args=None):
    ...
    if detector_name not in _REGISTRY:
        raise ValueError(
            f"Unknown detector: {detector_name}. Available: {list(_REGISTRY)}"
        )
